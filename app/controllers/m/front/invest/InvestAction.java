...
public class InvestAction extends BaseController {
	...
	/*	PageBean<v_front_all_bids> pageBean = new PageBean<v_front_all_bids>();
		pageBean = Invest.queryAllBids(Constants.SHOW_TYPE_1,
				currPage, pageSize, apr, amount, loanSchedule, startDate,
				endDate, loanType, minLevel, maxLevel, orderType, keywords,
				error);*/
	...
	public static void investDetail(long bidId, String showBox) {
		...
		render(bid, null, null, user, status, pageBean, null,
				uuid);
	}
	...
	public static void investVerifySMS(String sign,String uuid,String smscode){
		...
		if(!CaptchaUtil.checkUUID(uuid)){			
		 ...
		 Map<String, Object> resultMap =PaymentProxy.getInstance().investVerifySMS(error, Constants.PC,investId ,smscode);
