...
import models.t_rights_of_group;
...
public class RightGroup implements Serializable{
	 ...
	 * @return
	 */
	public static PageBean<t_right_groups> queryRightGroups(int currPage, int pageSize, String keyword, ErrorInfo error) {
		error.clear();
		...
		int count = 0;
		List<t_right_groups> page = new ArrayList<t_right_groups>();
		...
		try {
            ...
            for(Object[] record: resultlist){
            	t_right_groups  re = new t_right_groups();
            	re.uid = (Integer) record[0];
            	re.name = (String) record[1];
            	re.description= (String) record[2];
            	re.right_modules = (String) record[3];
            	page.add(re);
            }
		...
		PageBean<t_right_groups> bean = new PageBean<t_right_groups>();
